...
class SBMLModelHandler(ModelHandler):
    ...
    def modify_species(self, species_name, value):
        return super().modify_species(species_name, value)
...
class AMICIModelHandler(ModelHandler):
    def __init__(self, model_path):
        super().__init__(model_path)
        self.load_model()
    ...
    def load_model(self):
        ...
        self.model = self.amici_model.getModel()  # Assuming `getModel()` exists
...
class PerturbationHandler:
    """
    Handles applying perturbations to a (AMICI, maybe others?) model instance.
    """
    def __init__(self, model_handler, perturbations):
        """
    ...
    def apply_perturbations(self): 
        ...
        """
        for param, value in self.perturbations.get("parameters", {}).items():
            self.model_handler.modify_parameter(param, value)

        for species, value in self.perturbations.get("species", {}).items():
