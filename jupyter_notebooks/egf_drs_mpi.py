...
parser.add_argument('--sim_name',metavar='sim_name', help='insert exp name', default = 'egf_drs_test_mpi')
...
output_dose = os.path.join(output_path,'egf_'+str(float(args.egf)))
...
def find_dp_all(xoutS,species_all=species_all):
    ...
    if len(dp_all)!=0:
        dp_all_actual = list(np.array(dp_all)[data[dp_all]<1.25])
