...
timecourse('ppERK',rdata_new['x'],rdata_new['t'])
...
omics_input = 'OmicsData_extended.txt'
...
params_all.to_csv(os.path.join(wd,'initializer','params_10ai.txt'),sep='\t')
x6.to_csv(os.path.join(wd,'initializer','species_10ai.txt'),sep='\t',index=True, header=False)
...
writer.writeSBML(sbml_doc_10ai, os.path.join(wd,'SPARCED_10ai.xml'))
...
sbml_importer = amici.SbmlImporter(os.path.join(wd,'SPARCED_10ai.xml'))

model_output_dir_10a = str(model_output_dir+'_10ai')

model_path = os.path.join(wd,model_output_dir_10a)
